...
class regulasi_pajak_model extends MY_Model {
    ...
    function get_search_result_perpage($terms, $kategori, $jenis_dokumen_array, $tanggal, $tahun, $nomor, $method, $judul, $sort, $page, $perpage)
    {
        ...
        if($kategori == 'semua-kategori' && $jenis_dokumen_array[0] == 'semua-dokumen')
        {
            if($terms)
            {
                if($method == 'kalimat')
                {
                    if($judul == 0) $where = "body_final like '%$terms%'";
                    if($judul == 1) $where = "perihal like '%$terms%'";
                }
                if($method == 'atau')
                {
                    if($judul == 0) $where = "MATCH (body_final) AGAINST ('+$terms' IN BOOLEAN MODE)";
                    if($judul == 1) $where = "MATCH (perihal) AGAINST ('+$terms' IN BOOLEAN MODE)";
                }
    ...
    /*function get_search_result_all_old($terms)
    {
        $where = "MATCH (body_final) AGAINST ('+$terms') > 0";        
    ...
    function get_search_result_perpage($jenis_dokumen_lengkaps, $terms, $limit)
    {
        $i = 1;
        foreach($jenis_dokumen_lengkaps as $jenis_dokumen_lengkap)
        {
    ...
    function get_search_result_all_perpage($terms)
    {
        $where = "MATCH (body_final) AGAINST ('+$terms') > 0";        
